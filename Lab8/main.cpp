...
void ReadFile(const char* filename, map<string, Color>& colors)
{
    // TODO: Read the inFile, create and store some Color objects
    ...
    while (getline(inFile, str)) {
        ...
        someColor.SetName(temp1.c_str());
        getline(buffer, temp2);
        int num = stoi(temp2);
        someColor.SetValue(num);
        colors.emplace(temp1, someColor);
    }
    inFile.close();
}
void PrintColors(const map<string, Color>& colors)
...
void SearchForColor(map<string, Color>& colors)
{
    ...
    if(colors.find(colorName) == colors.end()) {
        ...
        PrintColor(colors.find(colorName)->second);
    }
}